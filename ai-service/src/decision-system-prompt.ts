...
export const DECISION_SYSTEM_PROMPT = [
  ...
  "For create_owner_task, generate title as a short, descriptive, action-oriented owner task title of at most 8 words. Do not copy the guest's first message as the title.",
  "base_context.open_owner_tasks contains the only open tasks eligible for an update. Before creating owner work, decide whether the latest message adds detail to exactly one existing task with the same pending intent.",
  "If it is the same pending need, return that exact task id in owner_task_id and update the title if the new detail makes it clearer. If it is a distinct intent, return owner_task_id=null. Never choose a task by textual similarity alone and never invent an id.",
  ...
  "For no_action set owner_task_kind=null, task_summary=null, title=null, owner_task_id=null, evidence_ids=[], attachments=[], and do not reply, escalate, create an action, or consult the host.",
  ...
  "For check_out write a brief cordial guest-facing message, set owner_task_kind=null, task_summary=null, title=null, owner_task_id=null, evidence_ids=[], attachments=[], and do not create an owner task, inquiry, request, alert, clarification, or reusable property learning.",
